...
class _Connection:
    ...
    async def _read_from_reticulum(self):
        """Read HDLC stream from Reticulum, emit decoded packets to on_tx."""
        buf = bytearray()
        while True:
    ...
    async def _write_to_reticulum(self):
        """Drain the RX queue, HDLC-encode, send to Reticulum TCP socket."""
        while True:
            ...
            try:
                ...
                log.info("RX packet %d bytes → Reticulum (total %d)", len(pkt), self.rx_bytes_sent)
    ...
    def put_rx(self, packet: bytes):
        ...
        if not self._closed:
            self._loop.call_soon_threadsafe(self._rx_queue.put_nowait, packet)
...
class Bridge:
    ...
    def __init__(self, host: str = DEFAULT_HOST, port: int = DEFAULT_PORT):
        self.host = host
        self.port = port
        self.on_tx_packet = None   # set to callable(packet: bytes)
    ...
    async def _handle(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        conn = _Connection(reader, writer, self.on_tx_packet)
